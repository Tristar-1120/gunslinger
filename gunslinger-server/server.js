...
function startRound(room) {
    ...
    setTimeout(() => {
        if (rooms.has(room.code) && room.state === 'playing') {
            ...
            // 5 second timeout - if no one shoots, kick both players
            setTimeout(() => {
                if (rooms.has(room.code) && room.state === 'playing' && !room.roundWinner) {
                    console.log(`Round timeout in room ${room.code} - kicking both players`);
                    
                    // Notify players they're being kicked for inactivity
...
setInterval(() => {
    ...
    for (let i = matchmakingQueue.length - 1; i >= 0; i--) {
        if (now - matchmakingQueue[i].joinedAt > QUEUE_TIMEOUT) {
            console.log(`Removing stale queue entry: ${matchmakingQueue[i].socketId}`);
